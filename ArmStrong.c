...
bool updateGyroValue(int &previousValue, int &timesSettled, bool &exit)
{
	...
	displayString(3, "%dm %ds till shutting down", obtainMinutes(deltaT), obtainSeconds(deltaT));
...
bool checkTouchValues(tSensors &touch, int &presses)
{
	...
	//// both touch sensors are pressed, check if it's a hold
	//else if (SensorValue[S1] == 1 && SensorValue[S2] == 1)
	//{
	//	int holdChecks = 0;
	...
	//		// one sensor released, not a hold
	//		else
	//			return true;
	//	}
	...
	else
	{
		...
		//{
		//	// waiting for touch sensor to be pressed
		//	while (time1[T4] < CLICK_TIME && SensorValue[touch] == 0) {}

		//	// waiting for touch sensor to be released
		//	while (time1[T4] < CLICK_TIME && SensorValue[touch] == 1) {}
...
void resetFingers()
{
	...
	motor[motorD] = -10;
	while(nMotorEncoder[motorD] > -45) {} //-35 works for the red shorter finger length pieces.
	motor[motorD] = 0;
	nMotorEncoder[motorD] = 0;
}
...
void pinch(float target, const float kp, const float kd, tMotor motorType){
	//	const int angleTargetPointerFinger = 70;
